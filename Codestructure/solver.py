...
def solve_system(
    ...
    def stress_ufl(u, theta, lmbda, mu, beta, theta_0):
        # Non-linear stress
        F = ufl.grad(u) + ufl.Identity(2)
        detF = ufl.det(F)
        C = ufl.dot(ufl.transpose(F), F) 
        C_inv = ufl.inv(C)
        
        return (mu * ufl.Identity(2) + (lmbda * ufl.ln(detF) - mu - beta * (theta - theta_0)) * C_inv)

    beta = matvec_ufl[2] * (2 * matvec_ufl[1] + 3 * matvec_ufl[0])
    stress_projector = ProjectStress(kernel_data.V, matvec_ufl[0], matvec_ufl[1], beta, theta_0, stress_ufl)
    ...
    for nt in range(1, n_dt + 1):
        ...
        while n_newton <= 30:
            # assemble system
            A, L = assemble_system(kernel_data, dt, matvec_ufl, theta_0)
