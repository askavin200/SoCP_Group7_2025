...
convert_4_order_voigt, convert_2_order_voigt, Cinv_dyad_Cinv
...
def stiffness_element_uu(matpar: list, dphi_i: np.ndarray, dphi_j: np.ndarray, alpha_gp: float, theta_np1k: np.ndarray, det_j: float,
    ...
    kuu_nl = (np.linalg.multi_dot([B_I.T, dS_dC_voigt, 2 * B_J]) + np.linalg.multi_dot([dphi_i.T, S, dphi_j]) * np.eye(2)) * alpha_gp * det_j
...
def stiffness_element_ut(dphi_i: np.ndarray, phi_j: np.ndarray, alpha_gp: float, det_j: float, beta, F: np.ndarray) -> np.ndarray:
    ...
    dS_dtheta = -beta * C_inv_voigt

    # evaluate B operator
    B_I = B_nonlinear(F, dphi_i)

    kut_nl = (np.dot(B_I.T, dS_dtheta) * phi_j) * alpha_gp * det_j

    return kut_nl


def stiffness_element_tu(matpar: list, phi_i: np.ndarray, dphi_i: np.ndarray, dphi_j: np.ndarray, alpha_gp: float, det_j: float, J: float,
                         F: np.ndarray, F_n: np.ndarray, dtheta: np.ndarray, dt: float, theta_np1k: np.ndarray, beta) -> np.ndarray:
    ...
    # evaluate rate of Cauchy green tensor C
    C_dot = np.dot(F.T, F) - np.dot(F_n.T, F_n) # Not dividing by dt since we took LCM for dt in Ktu 
    ...
    # dt term missing since we took LCM for dt in Ktu
    ke_1 = np.dot((phi_i * 0.5 * beta * theta_np1k * (np.dot(C_dot_voigt, dCinvdC_voigt) + C_inv_voigt)).T, 2 * B_J)

    Cinv_dyad_Cinv_Voigt = convert_4_order_voigt(Cinv_dyad_Cinv(C))
    ...
    ke_2 = k_t * J * dt * Li_Gradtheta_voigt @ np.dot((dCinvdC_voigt + Cinv_dyad_Cinv_Voigt), 2 * B_J)

    ktu_nl = (ke_1 + ke_2) * alpha_gp * det_j
...
def stiffness_element_tt(matpar: list, dt: float, phi_i: np.ndarray, phi_j: np.ndarray, dphi_i: np.ndarray, dphi_j: np.ndarray, alpha_gp: float,
    ...
    # dt term missing since we took LCM for dt in Ktu
    ktt_nl = (phi_i * (0.5 * beta * np.tensordot(C_inv, C_dot) + rho_0 * c_t) * phi_j
            + k_t * dt * J * np.linalg.multi_dot([dphi_i.T, C_inv, dphi_j])) * alpha_gp * det_j
...
def load_u(S_voigt: np.ndarray, b: np.ndarray, phi_i: np.ndarray, dphi_i: np.ndarray, alpha_gp: np.array, det_j: float, F: np.array):
    ...
    lu_nl = - (np.dot(B_nl_i.T, S_voigt) + phi_i * b) * alpha_gp * det_j

    return lu_nl


def load_t(matpar: list, theta_dot: np.ndarray, theta_np1k: float, dt: float, dthetadx_np1k: np.ndarray, rhor: np.ndarray,
    phi_i: np.ndarray, dphi_i: np.ndarray, alpha: np.array, det_j: float, J: float, F: np.array, F_n: np.array, beta):
    ...
    # evaluate rate of Cauchy green tensor C
    C_dot = np.dot(F.T, F) - np.dot(F_n.T, F_n) # Not dividing by dt since we took LCM for dt in L_t

    # dt term missing since we took LCM for dt in L_t
    lt_nl = -((rho_0 * c_t * theta_dot + 0.5 * beta * theta_np1k * np.tensordot(C_dot, C_inv) - rhor) * phi_i
              + k_t * J * np.linalg.multi_dot([dthetadx_np1k, C_inv, dphi_i])) * alpha * det_j
...
def assemble_system_elmt(
    ...
    # Extract material parameters and compute beta
    lhS = matvec[0]
    mhS = matvec[1]
    alpha_t = matvec[2]
    ...
    for igp in range(0, alpha.shape[0]):
        ...
        # evaluate deformation tensor (current)
        F = dudx_np1k[igp,:,:] + np.eye(2)
        # evaluate deformation tensor (previous)
        F_n = dudx_n[igp,:,:] + np.eye(2)
        ...
        # evaluate determinant of F
        J = np.linalg.det(F)

        # evaluate 2nd Piola stress (time n+1, iterate k) at quadrature points (Voigt notation!)
        ...
        for ii in range(0, nodes_per_elmt):
            ...
            # evaluate quantities at gauss point and split i components
            phi_i_gp = phi[0, igp, ii, 0]
            dphi_i_gp = dphi_x[:, igp, ii].T
            dthetadx_gp = dthetadx_np1k[igp, :]

            # evaluate load vector contributions
            l_u = load_u(S_voigt, vec_b[igp, :], phi_i_gp, dphi_i_gp, alpha[igp], det_j, F)
            l_t = load_t(matvec, thetadot_np1k[igp], theta_np1k[igp], dt, dthetadx_gp, vec_rhor[igp], phi_i_gp,
                         dphi_i_gp, alpha[igp], det_j, J, F, F_n, beta)
            ...
            for jj in range(0, nodes_per_elmt):
                ...
                # evaluate quantities at gauss point and split j components
                phi_j_gp = phi[0, igp, jj, 0]
                dphi_j_gp = dphi_x[:, igp, jj].T

                # evaluate local stiffness constributions
                k_uu = stiffness_element_uu(matvec, dphi_i_gp, dphi_j_gp, alpha[igp], theta_np1k[igp], det_j, theta0, beta, S, F)
                k_ut = stiffness_element_ut(dphi_i_gp, phi_j_gp, alpha[igp], det_j, beta, F)
                k_tu = stiffness_element_tu(matvec, phi_i_gp, dphi_i_gp, dphi_j_gp, alpha[igp], det_j, J, F, F_n, dthetadx_gp, dt,
                                            theta_np1k[igp], beta)
                k_tt = stiffness_element_tt(matvec, dt, phi_i_gp, phi_j_gp, dphi_i_gp, dphi_j_gp, alpha[igp], det_j, J, F, F_n, beta)
